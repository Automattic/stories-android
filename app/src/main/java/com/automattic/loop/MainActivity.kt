...
class MainActivity : AppCompatActivity(), MainFragment.OnFragmentInteractionListener {
    ...
    fun onStorySaveResult(event: StorySaveResult) {
        ...
        if (event.isSuccess()) {
            val text = String.format(
                getString(StoriesR.string.story_saving_snackbar_finished_successfully),
            ...
            val errorText = String.format(
                getString(StoriesR.string.story_saving_snackbar_finished_with_error),
                StoryRepository.getStoryAtIndex(event.storyIndex).title
            )
            ...
            snackbar.setAction(StoriesR.string.story_saving_failed_quick_action_manage) {
    ...
    fun onStorySaveStart(event: StorySaveProcessStart) {
        EventBus.getDefault().removeStickyEvent(event)
        val text = String.format(
            getString(StoriesR.string.story_saving_snackbar_started),
            StoryRepository.getStoryAtIndex(event.storyIndex).title
        )
        Snackbar.make(findViewById(android.R.id.content), text, Snackbar.LENGTH_SHORT).show()
    }
