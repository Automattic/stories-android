...
class DecoderSurface implements SurfaceTexture.OnFrameAvailableListener {
     ...
     */
    void drawImage(long presentationTime) {
...
//                    Log.d(TAG, "scale[0] = " + scale[0] + " scale[1] = " + scale[1]);
        ...
        if (filter != null) {
            ...
            framebufferObject.enable();
            GLES20.glClear(GL_COLOR_BUFFER_BIT);
            filter.draw(filterFramebufferObject.getTexName(), framebufferObject, presentationTime);
        }
        ...
        normalShader.draw(framebufferObject.getTexName(), null, presentationTime);
    }
